...
export function CollegeCard({ college, index, onVisit, whyItFits }: Props) {
  ...
  const { enabled: favEnabled, isFavorite, toggle } = useFavorites();
  const faved = isFavorite(college.id);
  return (
    <motion.article
      ...
      <div className="flex items-start justify-between gap-3 relative">
        ...
        {favEnabled && (
          <button
            ...
            aria-pressed={faved}
            className={`shrink-0 w-9 h-9 rounded-full flex items-center justify-center transition border ${
              faved
                ? "bg-primary/15 text-primary border-primary/30"
                : "bg-foreground/[0.04] text-muted-foreground hover:text-primary border-foreground/10 hover:border-primary/30"
            }`}
          >
