import { COLLEGES, FEELINGS, DIRECTIONS, ALL_STATES, ENVIRONMENTS, type College, type EnvironmentTag } from "./colleges";
...
export type AppSettings = {
  ...
  showOnlyTeaserToAnonymous?: boolean; // gate full results behind sign-in
};
...
function defaults(): ContentSnapshot {
  return {
    ...
    states: [...ALL_STATES],
    environments: ENVIRONMENTS.map((e) => ({ ...e, states: [...e.states] })),
    settings: { allowStateMismatch: false, showOnlyTeaserToAnonymous: true },
    version: 0,
...
export function loadContent(): ContentSnapshot {
  ...
  try {
    ...
    return {
      ...
      environments: parsed.environments ?? base.environments,
