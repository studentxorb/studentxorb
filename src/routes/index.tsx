...
type Step = "intro" | "feeling" | "transition1" | "environment" | "direction" | "location" | "loading" | "results";
...
function Index() {
  ...
  useEffect(() => {
    if (step === "transition1") {
      const t = setTimeout(() => setStep("environment"), 1600);
  ...
  const handleEnvironment = (envId: string, suggested: string[]) => {
    setEnvironment(envId);
    setSuggestedStates(suggested);
    setTimeout(() => setStep("direction"), 350);
  };
  ...
  // Progress dots
  const stepIndex = ["intro", "feeling", "environment", "direction", "location", "results"].indexOf(
      ...
      <header className="fixed top-0 inset-x-0 z-50 px-6 md:px-10 py-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {user ? (
            ...
            <>
              <Link to="/auth" search={{ mode: "signin", redirect: "/" }} className="text-xs uppercase tracking-[0.2em] text-muted-foreground hover:text-foreground transition">
                Sign in
              </Link>
              <Link to="/auth" search={{ mode: "signup", redirect: "/" }} className="text-xs px-3 py-1.5 rounded-full text-primary-foreground" style={{ background: "var(--gradient-aurora)" }}>
                Sign up
              </Link>
            </>
          )}
      ...
      <section className="max-w-6xl mx-auto px-2">
        ...
        <div className="grid md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            { i: MapPin, t: "Environment", d: "City, mountains, beaches?" },
            ...
            { i: GraduationCap, t: "Shortlist", d: "Keep the paths that fit." },
          ].map((s, i) => {
            const Icon = s.i;
            return (
              <motion.div
